...
void Xml::insert_at(int pos, Xml *x) {
    make_hole(pos);
    tree[pos] = x;
}
...
auto Xml::find_on_tree(Xml *check, Xml **res) const -> bool {
    ...
    for (int i = 0; i < n; i++)
        if (tree[i] && !tree[i]->is_xmlc() && tree[i]->find_on_tree(check, res)) return true;
    return false;
}
...
void Xml::to_buffer(Buffer &b) const {
    ...
    //  www --;
    if (name.get_value() > 1) b.push_back_elt(name, id, 2);
    b.finish_xml_print();
}
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            if (texte) {
                ...
                if (xx) { sf->get_id().add_attribute_but_rend(xx->get_id()); }
