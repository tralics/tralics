...
private:
    ...
    std::unordered_map<symcodes, std::function<std::string(symcodes, subtypes)>> &the_name_fns();
    std::unordered_map<symcodes, std::unordered_map<subtypes, std::string>> &     the_names();
};
