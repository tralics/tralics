...
public:
  ...
  auto fetch_beg_end_spec(bool k, bool incat, int &com_loc, bool &seen_dollar,
                          String &) -> int;
  ...
  auto space_or_underscore() const -> bool {
    return buf[ptr] == '_' || buf[ptr] == ' ';
  }
  ...
  auto is_special_end() const -> bool {
    return buf[ptr] == '\n' || buf[ptr] == '#' || buf[ptr] == '%';
  }
