...
void Parser::umlaut() {
    ...
    if ((!cur_cmd_chr.is_letter() && !cur_cmd_chr.is_other()) || cur_tok.char_val().get_value() > int(nb_characters)) { //
...
void Parser::T_figure_table(symcodes x, subtypes c) {
    ...
    if (c == 2) {
        ...
        the_stack.add_att_to_last(np_rend, x == figure_cmd ? np_figure : np_table);
...
void Parser::translate03() {
    ...
    case after_assignment_cmd:
        ...
        if (tracing_commands()) the_log << lg_startbracebs << "afterassignment: " << cur_tok << lg_endbrace;
        return;
