...
auto Math::chars_to_mb(Buffer &B, bool rec) const -> bool {
    ...
    for (;;) {
        ...
        if (w.is_space() || w.is_letter() || w.is_other()) {
            ...
            (Logger &)log_and_tty << lg_start << "First invalid token in math-to-string cmd=" << w.cmd << " chr=" << w.chr << lg_end;
            std::cout << "\n";
            return false;
        }
        ++L;
...
void Buffer::show_uncomplete(String m) {
    ...
    if (empty())
        (Logger &)log_and_tty << "No character found\n";
    else
        (Logger &)log_and_tty << "So far, got " << c_str() << "\n";
    reset();
    push_back("error");
}
