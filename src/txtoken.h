...
public:
    ...
    auto is_letter_other() const -> bool { return cmd == other_catcode || cmd == letter_catcode; }
    auto is_relax() const -> bool { return cmd == relax_cmd; }
    auto is_undef() const -> bool { return cmd == undef_cmd; }
    auto is_undef_or_relax() const -> bool { return cmd == undef_cmd || cmd == relax_cmd; }
    ...
    auto is_minus_sign() const -> bool { return cmd == other_catcode && chr == '-'; }
    ...
    auto is_ok_for_the() const -> bool { return cmd > min_internal && cmd < max_internal; }
    auto is_ok_for_xspace() const -> bool;
    auto is_protected() const -> bool { return cmd == userp_cmd || cmd == userlp_cmd || cmd == userpo_cmd || cmd == userlpo_cmd; }
    auto val_as_digit() const -> int { return chr - '0'; }
    void set_mathml() { cmd = math_xml_cmd; }
    auto is_math_openclosebetween() const -> bool { return cmd == mathbetween_cmd || cmd == mathopen_cmd || cmd == mathclose_cmd; }
    auto special_name() const -> String;
    auto name() const -> String;
    auto is_single_quote() const -> bool { return cmd == other_catcode && chr == '\''; }
...
public:
    ...
    auto is_a_brace() const -> bool { return OB_t_offset <= val && val < RB_limit; }
    auto is_a_left_brace() const -> bool { return OB_t_offset <= val && val < CB_t_offset; }
    auto is_OB_token() const -> bool { return OB_t_offset <= val && val < CB_t_offset; }
    auto is_CB_token() const -> bool { return CB_t_offset <= val && val < RB_limit; }
    auto is_math_shift() const -> bool { return dollar_t_offset <= val && val < dollar_limit; }
    auto is_space_token() const -> bool { return val == space_token_val || val == newline_token_val; }
    auto is_same_token(const Token &x) const -> bool { return val == x.val || (is_space_token() && x.is_space_token()); }
    ...
    auto is_letter(uchar c) const -> bool { return val == uint(letter_t_offset) + c; }
    auto is_digit_token() const -> bool { return val >= other_t_offset + '0' && val <= other_t_offset + '9'; }
    auto is_lowercase_token() const -> bool { return val >= letter_t_offset + 'a' && val <= letter_t_offset + 'z'; }
    ...
    auto is_dec_separator() const -> bool { return val == other_t_offset + ',' || val == other_t_offset + '.'; }
