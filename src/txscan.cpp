...
void Parser::E_input(subtypes q) {
    if (q == endinput_code) {
        if (tracing_commands()) spdlog::trace("{{\\{}}}", "endinput");
        force_eof = true;
        return;
    }
    if (q == end_all_input_code) {
        if (tracing_commands()) spdlog::trace("{{\\{}}}", "endallinput");
        for (auto &i : cur_input_stack) i->destroy();
        force_eof = true;
        return;
    }
    if (q == scantokens_code) {
        if (tracing_commands()) spdlog::trace("{{\\{}}}", "scantokens");
        TokenList L = scan_general_text();
        T_scantokens(L);
        return;
    }
...
void Parser::M_prefixed_aux(bool gbl) {
    ...
    case set_mathchar_cmd: // set a slot in the math font table
    {
        ...
        scan_optional_equals();
        flush_buffer();
        math_data.math_chars[static_cast<uchar>(v)][k] = sT_arg_nopar();
        return;
    }
    case register_cmd:
    case advance_cmd:
...
void Parser::token_show(int what, Buffer &B) {
    ...
    if (lg) spdlog::info("{}{}.", prefix, B.convert_to_log_encoding());
}
...
void Parser::token_for_show(const CmdChr &val) {
    ...
    spdlog::trace("{}", B.convert_to_log_encoding());
}

auto Parser::token_for_show_str(const CmdChr &val) -> std::string {
    Buffer B;
    B.clear();
    token_for_show(false, val, B);
...
void Parser::E_convert() {
    ...
    case tralicsversion_code: B.append(the_main.tralics_version); break;
