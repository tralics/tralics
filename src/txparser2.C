...
void XkvSetkeys::find_pointer(const std::string &Key) { local_buf << bf_reset << "XKV@" << xkv_header << Key << "@value"; }
...
void Parser::E_parse_encoding(bool vb, subtypes what) {
    ...
    Token t = Token(other_t_offset, codepoint(static_cast<unsigned>(r)));
    back_input(t);
}
