...
void Parser::load_latex()
{
  ...
  L.insert("\\def\\@x@tag#1{\\formulaattribute{tag}{(#1)}}");
  L.insert("\\def\\@y@tag#1{\\formulaattribute{tag}{#1}}");
...
auto config_ns::assign_att(String A, String B) -> bool {
  ...
  case 'b':
   ...
   if (strcmp(A,"border_bottom_width")==0)
     { the_names[np_border_bottomw] = Istring(B); return true; }
   if (strcmp(A,"border_top_width")==0)
     { the_names[np_border_topw] = Istring(B); return true; }
   if (strcmp(A,"border_left_width")==0)
     { the_names[np_border_leftw] = Istring(B); return true; }
   if (strcmp(A,"border_right_width")==0)
     { the_names[np_border_rightw] = Istring(B); return true; }
    return false;
  case 'c':
