...
void Parser::bad_end_env(int cl) {
    local_buf << bf_reset << "End of data reached while scanning end of environment " << get_cur_env_name() << "\nscanning started line "
              << cl;
...
void Parser::missing_close_brace(int cl) {
    local_buf << bf_reset << "End of data reached while scanning a group;\n"
...
void Parser::bad_delimited(int cl, Token x) {
    local_buf << bf_reset << "End of data reached while scanning argument of " << err_tok.tok_to_str() << "\nscanning started at line "
              << cl;
...
void Parser::get_new_command_aux(const TokenList &a) {
    local_buf << bf_reset << "Only one token allowed in argument list of " << err_tok << "\n";
...
void Parser::missing_open_brace() {
    local_buf << bf_reset << "A left brace was mandatory here, so I've put one in.\n"
...
void Parser::lost_if(Token T, int l) {
    local_buf << bf_reset << "End of data reached while scanning \\fi"
...
void Parser::bad_nbargs(int k) {
    ...
    local_buf << bf_reset << "Bad syntax in optional argument\n" << S << "\nI will assume that the command takes no argument";
...
void Parser::bad_definition(Token name, int nb) {
    local_buf << bf_reset << "Error while scanning definition of " << name.tok_to_str();
...
void Parser::bad_csname(bool orig) {
    if (cur_tok.is_invalid())
        ...
        if (orig)
            ...
            local_buf << " found while scanning " << err_tok.tok_to_str();
    }
    signal_error(err_tok, "missing endcsname");
}

void Parser::bad_counter0() {
    local_buf << bf_reset << "Invalid token " << cur_tok.tok_to_str() << " found in counter name";
    signal_error(err_tok, "invalid cmd");
}

void Parser::bad_counter1(const Buffer &B, Equivalent &E) {
    local_buf << bf_reset << (E.is_undefined() ? "Unknown counter `" : "Invalid counter `") << B.c_str(2) << "'";
...
void Parser::undefined_mac() {
    ...
    local_buf << bf_reset << "Undefined command " << cur_tok.tok_to_str();
    if (!cur_cmd_chr.is_undef()) local_buf << "; command code = " << cur_cmd_chr.get_cmd();
    if (noxml) {
...
void Parser::short_verb_error(Token Tfe, Token t, int x) {
    ...
    if (x == 3)
        local_buf << "Not a short verb " << t;
    else if (x == 2)
        local_buf << "Already a short verb " << t;
    else if (t.is_null())
        local_buf << "One argument required";
    else
        local_buf << "A one-char control sequence is needed instead of " << t;
...
void Parser::counter_overflow(Token T, int n, int nmax) {
    local_buf << bf_reset << "Illegal counter value " << n << " for " << T.tok_to_str() << "\n";
    if (n <= 0)
        local_buf << "Value must be positive";
    else
        local_buf << "Value must be at most " << nmax;
    signal_error(T, "counter overflow");
}

void Parser::bad_redefinition(int rd, Token T) {
    local_buf << bf_reset << err_tok.tok_to_str();
    local_buf << ": Cannot define " << T.tok_to_str() << "; token is "
