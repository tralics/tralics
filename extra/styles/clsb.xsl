...
-->
<!-- $Id: clsb.xsl,v 1.1 2008/09/25 15:49:14 grimm Exp $ -->
...
<xsl:template match="div1" mode="normal">
  <h2> 
    <xsl:call-template name="id"/>
    <xsl:call-template name="calculateNumberSpace"/>
    <xsl:apply-templates select="head" mode="full"/> 
...
<xsl:template match="list/label" mode="dl">
  <b><xsl:apply-templates/></b>
</xsl:template>

<xsl:template match="item">
  <xsl:choose>
    <xsl:when test="parent::list/@type='description' and
		    preceding-sibling::label"  >
      <dt><xsl:apply-templates select="preceding-sibling::label[1]" mode="dl"/> </dt>
      <dd>
	<xsl:call-template name="id"/>
	<xsl:apply-templates/> 
      </dd>
...
<xsl:template match='subfigure' mode="xref">
   <xsl:call-template name="calculateFigureNumber"/>
</xsl:template>

<!-- Case of a subfigure as a table cell -->
<xsl:template match='cell' mode="xref">
  <xsl:choose>
    <xsl:when test="@id-text">
    <xsl:value-of select="@id-text"/>
    </xsl:when>
...
<xsl:template match="subfigure">
  <div class='subfigure'>
    <table>
	  ...
	  <xsl:apply-templates mode="xref" select="."/>
	  <xsl:text>) </xsl:text>
	  <xsl:apply-templates mode="full" select="head"/>
      </td></tr>
    </table>
  </div>
</xsl:template>
...
<xsl:template match="figure[@rend='subfigure']">
    <table style='caption-side:bottom' width="80%" id="{./@id}">
      <caption>
	<strong>
	  <xsl:text>Figure </xsl:text>
...
<xsl:template match="otheorem">
  <div class='theorem-{@type}'><i>
    <p><a><b>
	  <xsl:call-template name="id"/>
...
<xsl:template match="div1" mode="first-par">
...
<xsl:template match="theorem" mode="first-par">
    <a style="font-weight: bold;font-style:normal;">
      <xsl:call-template name="id"/>
      <xsl:apply-templates select="head" mode="full"/>
      <xsl:text> </xsl:text>
